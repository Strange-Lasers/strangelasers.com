...
This is a design draft held for content review, not an approved release. Preserve the work on `feat/about-us`; use `git worktree list` to locate its working directory. Do not merge or deploy the draft while its content is unfinished. Only the polished final page and approved homepage integration belong on `main`, not the preview files or controls.
...
- Obtain approval for the final content and integration before merging; pushing and deploying require separate authorization
