...
Eleventy builds Nunjucks templates from `src/` into the generated `dist/` directory. Keep shared copy in `src/_data/site.json`, people in `src/_data/people.json`, and reusable markup in `src/_includes/`. Edit the source templates and data instead of generated HTML. Start Docker, then run `npm ci` and `npm run cover:install` for a fresh checkout, `npm run dev` for local preview, and `npm run check` before committing site changes. Native screenshot comparisons also need `npm run screenshots:install`. Every filesystem build regenerates `docs/screenshots/cover.png` at 0 ms with motion disabled in the pinned Linux container; include a changed cover with its source changes. CI checks that the committed PNG matches the generated image. The explicit passthrough list in `eleventy.config.mjs` owns which static assets reach the deployment bundle.
